...
public class BizOrderServiceImpl extends BaseService implements BizOrderService {
	...
	if (traderInfo.isManNotify(item.getCarrierType()) || traderInfo.isAutoNotify(item.getCarrierType())) {// 预成功
	    bizOrder.setNotifyStatus(Constants.BizOrder.NOTIFY_INIT);
	} else {
	    bizOrder.setNotifyStatus(Constants.BizOrder.NOTIFY_NORMAL);
	}
	if (bizOrder.isVoucher()) {// 积分
	    bizOrder.setVcodeStatus(Constants.BizOrder.VCODE_INIT);
	}
	...
	try {
		...
		if (notifyStatus == Constants.BizOrder.NOTIFY_INIT || notifyStatus == Constants.BizOrder.NOTIFY_CHARGING
			|| notifyStatus == Constants.BizOrder.NOTIFY_UNKNOWN) {// 预成功
		    bizOrder.setNotifyStatus(Constants.BizOrder.NOTIFY_SUCCESS);
		}
	    }
	    if (bizOrder.isVoucher()) {
		Integer vcodeStatus = bizOrder.getVcodeStatus();
		if (vcodeStatus == Constants.BizOrder.VCODE_INIT) {// 积分
		    bizOrder.setVcodeStatus(Constants.BizOrder.VCODE_SENT);
		}
	    }
	...
	try {
	    ...
	    if (bizOrder.isManNotify()) {
		Integer notifyStatus = bizOrder.getNotifyStatus();
		if (notifyStatus == Constants.BizOrder.NOTIFY_INIT) {
    ...
    public int countNowOrder() {
        Integer result = null;
        try {
            result = bizOrderDAO.countNowOrder();
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return  result;
    ...
    public int countSuccessOrder() {
        Integer result = null;
        try {
            result = bizOrderDAO.countSuccessOrder();
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return  result;
    ...
    public int countThreeDayOrder() {
        Integer result = null;
        try {
            result = bizOrderDAO.countThreeDayOrder();
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return  result;
...
//            OrderReport report = DateTool.dateFilterOrderReport(orderReport);
            result = bizOrderDAO.selectReportCount(orderReport);
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return  result;
