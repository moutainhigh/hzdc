...
public class StockServiceImpl extends BaseService implements StockService {
	...
	try {
	    int row = stockDAO.updateByPrimaryKeySelective(stock);
	    result.setSuccess(row > 0);
	    result.setModule(row > 0);
	} catch (SQLException ex) {
	    result.setResultMsg("更新stock失败 stockId：" + stock.getId());
	    logger.error("updateStock error", ex);
	}
	return result;
	}
	...
	public Result<Stock> getStockByCardId(Long supplyTraderId, String cardId) {
		...
		try {
			StockExample example = new StockExample();
			example.createCriteria().andSupplyTraderIdEqualTo(supplyTraderId).andCardIdEqualTo(cardId)
					.andStatusEqualTo(Constants.Stock.STATUS_DELIVERY);
			List<Stock> stocks = (List<Stock>) stockDAO.selectByExample(example);
			if (stocks.size() == 1) {
				result.setModule(stocks.get(0));
				result.setSuccess(true);
			} else {
				result.setResultMsg("卡券不存在、或有多笔存在 cardId：" + cardId);
			}
		} catch (Exception ex) {
			result.setResultMsg("查询库存是失败  msg : " + ex.getMessage());
			logger.error("getStockByCardId error cardId : " + cardId, ex);
		}
		return result;
	}
