...
public class UserAlertServiceImpl extends BaseTokenService implements UserAlertService {
	...
	@Override
	public Result<Boolean> bizOrderRequest(final BizOrder bizOrder) {
		Result<Boolean> result = new Result<Boolean>();
			...
			public void run() {
				try {
					...
					if (status == Constants.BizOrder.STATUS_INIT) {
						...
						map.put("carrierType", bizOrder.getCarrierType() + "");
	...
	@Override
	public Result<Boolean> supplyOrderRequest(final SupplyOrder supplyOrder) {
		Result<Boolean> result = new Result<Boolean>();
			...
			public void run() {
				try {
					...
					if (status == Constants.SupplyOrder.STATUS_INIT) {
						...
						map.put("carrierType", getCarrierType(supplyOrder.getBizId()) + "");
	...
	public Boolean smsNotifyRequest(String mobile, String content) {
		String url = alert_url + "/sms/channel";
		Map<String, String> map = getRootData(0l);
	...
	private int getCarrierType(int bizId) {
		if (Constants.BizInfo.CODE_FLOW_UNICOM == bizId || Constants.BizInfo.CODE_PHONE_UNICOM == bizId) {
			return Constants.Item.CARRIER_TYPE_UNICOM;
		} else if (Constants.BizInfo.CODE_FLOW_MOBILE == bizId || Constants.BizInfo.CODE_PHONE_MOBILE == bizId) {
			return Constants.Item.CARRIER_TYPE_MOBILE;
		} else if (Constants.BizInfo.CODE_FLOW_TELECOM == bizId || Constants.BizInfo.CODE_PHONE_TELECOM == bizId) {
			return Constants.Item.CARRIER_TYPE_TELECOM;
		}
		return 0;
	}
}
