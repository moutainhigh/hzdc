...
public class BizOrder implements Serializable {
    ...
    public String getNotifyStatusDesc() {
        ...
        if (Constants.BizOrder.NOTIFY_INIT == notifyStatus) {
            ...
            result = Constants.BizOrder.NOTIFY_UNKNOWN_DESC;
        }
        return result;
    }

    public String getVcodeStatusDesc() {
        String result = null;
        if (vcodeStatus == null) {
            return null;
        }

        if (Constants.BizOrder.VCODE_INIT == vcodeStatus) {
            result = Constants.BizOrder.VCODE_INIT_DESC;
        } else if (Constants.BizOrder.VCODE_SENT == vcodeStatus) {
            result = Constants.BizOrder.VCODE_SENT_DESC;
        } else if (Constants.BizOrder.VCODE_USED == vcodeStatus) {
            result = Constants.BizOrder.VCODE_USED_DESC;
        } else if (Constants.BizOrder.VCODE_INVALID == vcodeStatus) {
            result = Constants.BizOrder.VCODE_INVALID_DESC;
        } else if (Constants.BizOrder.VCODE_REFUND == vcodeStatus) {
            result = Constants.BizOrder.VCODE_REFUND_DESC;
        }
        return result;
    }
    ...
    public boolean canSendCode() {
        if (status == null || vcodeStatus == null) {
            return false;
        }
        return Constants.BizOrder.STATUS_SUCCESS == status && Constants.BizOrder.VCODE_SENT == vcodeStatus;
    }

    public boolean canRecordCode() {
        if (status == null || vcodeStatus == null) {
            return false;
        }
        return Constants.BizOrder.STATUS_SUCCESS == status
                && (Constants.BizOrder.VCODE_SENT == vcodeStatus || Constants.BizOrder.VCODE_USED == vcodeStatus);
    }

    public boolean canInvalidCode() {
        if (status == null || vcodeStatus == null) {
            return true;
        }
        if (Constants.BizOrder.STATUS_SUCCESS == status
                && (Constants.BizOrder.VCODE_USED == vcodeStatus || Constants.BizOrder.VCODE_REFUND == vcodeStatus)) {
            return false;
        }
        return true;
    }

    public boolean canRefundCode() {
        if (status == null || vcodeStatus == null) {
            return false;
        }
        return Constants.BizOrder.STATUS_SUCCESS == status && Constants.BizOrder.VCODE_INVALID == vcodeStatus;
    }
    ...
    public boolean notifyGreen() {
        ...
        return Constants.BizOrder.NOTIFY_SUCCESS == notifyStatus || Constants.BizOrder.VCODE_SENT == notifyStatus
                || Constants.BizOrder.VCODE_USED == notifyStatus;
    }

    public boolean vcodeRed() {
        if (vcodeStatus == null) {
            return false;
        }
        return Constants.BizOrder.VCODE_INVALID == vcodeStatus;
    }

    public boolean vcodeGreen() {
        if (vcodeStatus == null) {
            return false;
        }
        return Constants.BizOrder.VCODE_SENT == vcodeStatus || Constants.BizOrder.VCODE_USED == vcodeStatus;
    }
    ...
    public boolean isVoucher() {
        return Constants.BizInfo.CODE_JFDH_VOUCHER == bizId;
    }
